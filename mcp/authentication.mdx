...
<Steps>
  ...
  <Step title="Name your workspace">
    The client opens NeetoPlaydash in your browser and asks which workspace to connect. Enter the subdomain: for `acme.neetoplaydash.com`, enter `acme`. See [Workspace subdomain](/api/workspace-subdomain).
...
Generate one from your workspace's API keys screen; see the [help centre article](https://help.neetoplaydash.com/articles/api-keys). It is the same key the [REST API](/api/authentication) uses.
...
| --- | --- | --- |
| [REST API](/api/authentication) | `X-Api-Key` header | The key's workspace |
