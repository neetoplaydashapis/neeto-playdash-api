...
import InsightsShowFlags from '/snippets/cli/insights/show.mdx'
...
For fields and response details, see the [API reference](/api/insights/get).
...
Shows the project's daily and overall run and test insights for a date range. `get` is
still accepted as an alias for `show`.
...
```bash
neetoplaydash insights show pdqmxlz --start-date 2026-07-15 --branch main
```
...
```bash
neetoplaydash insights show pdqmxlz --tags nightly,smoke --timezone America/New_York
```
