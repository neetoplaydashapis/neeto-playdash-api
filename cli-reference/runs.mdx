...
import RunsShowFlags from '/snippets/cli/runs/show.mdx'
...
For fields and response details, see the [API reference](/api/runs/list).
...
The server caps this endpoint at 50 records per page. A larger `--page-size` prints a warning on stderr, and the server clamps the page to 50. Check `pagination.page_size` in the response for the size actually applied.
...
```json Sample output
{
  ...
  "breadcrumbs": [
    { "label": "Show run", "command": "neetoplaydash runs show <run_id> --project <project_id>" },
...
Shows one run with its summary, aggregate statistics, and the spec files it executed, each with the tests grouped under it. Use it when you want the shape of a whole run in a single call rather than paging through its test entities. `get` is still accepted as an alias for `show`.
...
```bash
neetoplaydash runs show kftwnab --project pdqmxlz
```
...
See the [API reference](/api/runs/get) for the full response.
