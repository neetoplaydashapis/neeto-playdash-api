...
import TestEntitiesShowFlags from '/snippets/cli/test-entities/show.mdx'
...
For fields and response details, see the [API reference](/api/test-entities/list).
  ...
  "breadcrumbs": [
    {
      ...
      "command": "neetoplaydash test-entities show <test_entity_id> --project <project_id> --run <run_id>"
    }
  ],
  "pagination": {
    "total_records": 1,
...
Shows one test entity with everything the run recorded for it. For a **test** that means the outcomes, the attempts inside each outcome, the Playwright error output, and time-limited links to the screenshots, videos, traces, and Lighthouse reports captured. For a **spec** it means the list of tests the run recorded in that file. `get` is still accepted as an alias for `show`.
...
See the [API reference](/api/test-entities/get) for the full response.
