...
`--subdomain <name>` is the fourth global flag. It picks the signed-in workspace a command targets and does not change the format. See [Working with multiple workspaces](/cli/authentication#working-with-multiple-workspaces).
...
Defaults and caps differ by command. A `--page-size` above the cap prints a warning on stderr, and the server clamps the page to the cap. Read `pagination.page_size` in the response to see what you actually got.

```text
Warning: --page-size 200 is above this command's maximum of 50; the server returns at most 50 records per page.
```

The CLI sends `page_size` only when you pass `--page-size`, so the flag's own default shows as `0` in the reference tables. The defaults below are what the server applies when the flag is absent.
