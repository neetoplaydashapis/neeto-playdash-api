...
The message is the server's own. See [HTTP response codes](/api/response-codes) for what each status means. A 401 means the stored session is no longer valid: sign in again with `neetoplaydash login`. `insights show` returns a 400 with `Invalid date range format` for a date that is not `YYYY-MM-DD`, and `Insights are available only for the most recent 3 months` with the earliest date you may use when `--start-date` is too old.
