...
Errors from the server come back with the server's own message. The HTTP status is the first clue - see [HTTP response codes](/api/response-codes) for what each one means. A 401 means the stored session is no longer valid: sign in again with `neetoplaydash login`.
